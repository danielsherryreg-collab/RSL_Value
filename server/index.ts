...
const analysisSchema=z.object({screens:z.array(z.object({slotId:z.string().max(40),label:z.string().max(100),image:z.string().startsWith('data:image/').max(1_500_000)})).min(1).max(40)});
...
const offerSchema=z.object({title:z.string().trim().max(100),description:z.string().trim().max(1200),images:z.array(z.string().startsWith('data:image/').max(10_000_000)).min(10).max(40),screenshotSlotIds:z.array(z.string().max(50)).min(10).max(40),offerPriceRub:z.number().int().min(0).max(10_000_000),estimatedPriceRub:z.number().int().min(0).max(10_000_000),accountData:accountDataSchema});
...
app.post('/api/offers',telegramAuth,async(req,res)=>{
  const parsed=offerSchema.safeParse(req.body);if(!parsed.success)return res.status(400).json({error:'Не удалось приложить скриншот. Попробуйте выбрать изображение ещё раз'});if(!['clan_boss','hydra','profile_facebook'].every(id=>parsed.data.screenshotSlotIds.includes(id)))return res.status(400).json({error:'Добавьте обязательные скриншоты: Клановый босс, Гидра и Профиль / привязка Facebook'});
  ...
  const clean=deduplicateOpenOffers(await getOffers());const items=clean.items;const incoming:StoreOffer={id:`offer_${Date.now()}_${crypto.randomBytes(5).toString('hex')}`,sellerTelegramUserId:req.telegramUser!.id,sellerUsername:req.telegramUser!.username,sellerDisplayName:req.telegramUser!.first_name,title:parsed.data.title||'Аккаунт RSL без названия',description:parsed.data.description||'Описание не заполнено — требуется уточнение модератором',images:parsed.data.images,screenshotSlotIds:parsed.data.screenshotSlotIds,offerPriceRub:parsed.data.offerPriceRub,estimatedPriceRub:parsed.data.estimatedPriceRub,accountData:parsed.data.accountData,status:'pending',createdAt:new Date().toISOString(),moderationFlags:flags};
