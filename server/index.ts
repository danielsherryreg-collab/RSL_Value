...
const offerSchema=z.object({title:z.string().trim().max(100),description:z.string().trim().max(1200),images:z.array(z.string().startsWith('data:image/').max(10_000_000)).min(1).max(3),offerPriceRub:z.number().int().min(0).max(10_000_000),estimatedPriceRub:z.number().int().min(0).max(10_000_000)});
...
app.post('/api/offers',telegramAuth,async(req,res)=>{const parsed=offerSchema.safeParse(req.body);if(!parsed.success)return res.status(400).json({error:'Не удалось приложить скриншот. Попробуйте выбрать изображение ещё раз'});const flags:string[]=[];if(parsed.data.title.length<5)flags.push('Короткое или отсутствующее название');if(parsed.data.description.length<10)flags.push('Описание требует уточнения');if(parsed.data.offerPriceRub<1000)flags.push('Цена ниже рекомендуемого минимума');const items=await getOffers();const item:StoreOffer={id:`offer_${Date.now()}_${crypto.randomBytes(5).toString('hex')}`,sellerTelegramUserId:req.telegramUser!.id,sellerUsername:req.telegramUser!.username,title:parsed.data.title||'Аккаунт RSL без названия',description:parsed.data.description||'Описание не заполнено — требуется уточнение модератором',images:parsed.data.images,offerPriceRub:parsed.data.offerPriceRub,estimatedPriceRub:parsed.data.estimatedPriceRub,status:'pending',createdAt:new Date().toISOString(),moderationFlags:flags};items.push(item);await saveOffers(items);const appUrl=publicAppUrl();for(const id of adminIds()){if(appUrl)sendMessage(id,`Новый оффер: ${item.title}\nПродавец хочет: ${item.offerPriceRub.toLocaleString('ru-RU')} ₽${flags.length?`\n⚠️ Требует внимания: ${flags.join('; ')}`:''}`,{inline_keyboard:[[{text:'Проверить оффер',web_app:{url:`${appUrl}/?tgWebAppStartParam=${item.id}#market`}}]]}).catch(err=>console.error('Admin notification failed',err))}res.status(201).json(item)});
