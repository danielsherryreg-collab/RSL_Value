...
app.post('/api/telegram/webhook', async (req,res) => {
  ...
  if(callback&&appUrl){
    ...
    if(isAdmin&&data.startsWith('admin:view_offer:')){
      const item=(await getOffers()).find(x=>x.id===data.slice('admin:view_offer:'.length)&&x.status==='pending');
      if(item)await sendMessage(chatId,'После просмотра данных и скриншотов можно подготовить публикацию:',{inline_keyboard:[[{text:'📢 Опубликовать',callback_data:`admin:publish_offer:${item.id}`}]]});
    }
  }
