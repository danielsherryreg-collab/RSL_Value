...
export async function sendImageAlbums(chatId:number, images:string[]) {
  for(let start=0;start<images.length;start+=10){
    ...
    if(chunk.length===1){const match=chunk[0].match(/^data:(image\/[^;]+);base64,(.+)$/);if(!match)throw new Error('Invalid image data');form.append('photo',new Blob([Buffer.from(match[2],'base64')],{type:match[1]}),'screenshot.jpg');const token=botToken();if(!token)throw new Error('Telegram bot token is not configured');const response=await fetch(`${apiBase(token)}/sendPhoto`,{method:'POST',body:form});const result=await response.json() as {ok:boolean;description?:string};if(!result.ok)throw new Error(result.description||'Telegram photo error');continue}
    ...
    form.set('media',JSON.stringify(media));const token=botToken();if(!token)throw new Error('Telegram bot token is not configured');const response=await fetch(`${apiBase(token)}/sendMediaGroup`,{method:'POST',body:form});const result=await response.json() as {ok:boolean;description?:string};if(!result.ok)throw new Error(result.description||'Telegram media error');
  }
}
