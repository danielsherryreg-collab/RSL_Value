...
const token=process.env.RSL_VALUE_BOT_TOKEN||process.env.BOT_TOKEN; const url=process.env.PUBLIC_APP_URL;
if(!token||!url) throw new Error('Set RSL_VALUE_BOT_TOKEN or BOT_TOKEN and PUBLIC_APP_URL in .env');
...
const configured=await call('getChatMenuButton',{}) as {result?:{text?:string}};
if(configured.result?.text!=='Оценить\\Купить') throw new Error('Telegram menu button verification failed');
