...
export default function OfferMarket({ screens, estimateRub, accountData, mode = 'all' }: { screens: Screen[]; estimateRub: number; accountData?: NonNullable<StoreOffer['accountData']>; mode?: 'all' | 'sell' | 'buy' }) {
  ...
  return <section className={`market offer-market ${mode === 'buy' ? 'buy-only' : mode === 'sell' ? 'sell-only' : ''}`} id="market">
    ...
    <div className="market-layout">
      ...
      <div className="market-feed"><div className="feed-head"><b>Магазин аккаунтов</b><span>{offers.length} предложений</span></div>{!offers.length && <div className="feed-empty">Принятых офферов пока нет. После подтверждения первый аккаунт появится здесь автоматически.</div>}{offers.map(item => {const active=Math.min(slides[item.id]||0,Math.max(0,item.images.length-1));const src=item.images[active];return <article className="store-card" key={item.id}><div className="auction-images store-carousel" onTouchStart={event=>{touchStart.current=event.touches[0].clientX}} onTouchEnd={event=>{const delta=event.changedTouches[0].clientX-touchStart.current;if(Math.abs(delta)>45)moveSlide(item,delta>0?-1:1)}}>{src&&<button type="button" className="image-preview-button" onClick={()=>setZoom({src,title:`${item.title} · скриншот ${active+1}`})}><img src={src} alt={`Скриншот аккаунта ${active+1}`}/></button>}<span>В ПРОДАЖЕ</span>{item.images.length>1&&<><button type="button" className="carousel-arrow previous" aria-label="Предыдущий скриншот" onClick={()=>moveSlide(item,-1)}>‹</button><button type="button" className="carousel-arrow next" aria-label="Следующий скриншот" onClick={()=>moveSlide(item,1)}>›</button><b className="carousel-count">{active+1} / {item.images.length}</b></>}</div><div className="auction-body"><div className="seller">Проверено RSL Value</div><h3>{item.title}</h3><p>{item.description}</p><div className="store-price"><small>Цена в магазине</small><b>{fmt(item.retailPriceRub || item.offerPriceRub)} ₽</b>{typeof item.commissionRub === 'number' && <span>Сумма продавцу {fmt(item.offerPriceRub)} ₽ + комиссия {fmt(item.commissionRub)} ₽</span>}</div><button className="buy-interest" onClick={() => window.Telegram?.WebApp.openTelegramLink?.('https://t.me/rsl_value_bot')}>Заинтересовал аккаунт <span>→</span></button></div></article>})}</div>
    </div>
