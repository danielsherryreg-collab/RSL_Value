...
const screenOfferId=()=>new URLSearchParams(location.search).get('screens')||decodeURIComponent(location.hash.slice('#screens/'.length).split(/[?&]/,1)[0]);
const viewFromHash=():View=>new URLSearchParams(location.search).has('screens')||location.hash.startsWith('#screens/')?'screens':location.hash==='#estimate'?'estimate':location.hash==='#market'?'market':'home';
...
const initial:FormState={power:0,level:0,legendaries:0,mythicals:0,voidLegendaries:0,sixStar:0,greatHall:0,factionWars:0,clanBoss:'normal',hydra:'none',arena:'bronze',gems:0,energy:0,silver:0,sacred:0,voidShards:0,legendaryBooks:0,champions:[]};
...
function Field({label,value,onChange,step=1,suffix,optional=false}:{label:string;value:number;onChange:(n:number)=>void;step?:number;suffix?:string;optional?:boolean}){
  const[draft,setDraft]=useState(String(value));useEffect(()=>setDraft(String(value)),[value]);
  return <label className="field"><span>{label}</span><div><input type="number" min="0" step={step} value={draft} onChange={event=>{const next=event.target.value;setDraft(next);onChange(next===''?0:Math.max(0,Number(next)))}}/>{optional&&<em>необязательно</em>}{suffix&&<b>{suffix}</b>}</div></label>;
}
...
function Header(_props:{view:View}){return <header className="topbar"><a className="logo" href="#home"><img className="logo-mark" src="/images/raid-store-full-logo.png" alt="Логотип RAID STORE"/><b>RAID <span>STORE</span></b></a><a className="ghost header-buy" href="#market">Купить аккаунт</a></header>}
function Footer(){return <footer><div className="logo"><img className="logo-mark" src="/images/raid-store-full-logo.png" alt="Логотип RAID STORE"/><b>RAID <span>STORE</span></b></div><p>Оценка аккаунтов RAID: Shadow Legends. Не связан с Plarium Global Ltd.</p><a href="#home">Наверх ↑</a></footer>}

function Home(){return <main className="home-page"><section className="hero home-hero"><div className="hero-copy"><div className="kicker"><span>●</span> RAID STORE / ОЦЕНКА АККАУНТОВ</div><h1><em>ОЦЕНИ</em><br/>АККАУНТ.<br/><small>RAID SHADOW LEGENDS</small></h1><div className="hero-line"/><p>Загрузи полный комплект скриншотов. Мы разберём героев, прогресс, ресурсы, подземелья и экипировку.</p><div className="home-actions"><a className="primary" href="#estimate">Оценка <span>→</span></a><a className="secondary home-buy" href="#market">Купить <span>→</span></a></div></div><div className="home-mark"><i>R</i><span>RAID STORE</span></div></section><section className="how home-how"><div className="section-title"><div><span>02 / МЕТОДИКА</span><h2>Что влияет на цену</h2></div></div><div className="factor-grid">{[['01','Редкие герои','Мифические и Войд-легендарные герои создают основную ценность коллекции.'],['02','Игровой прогресс','КБ, арена, Войны фракций и Большой зал показывают готовность к эндгейму.'],['03','Запас ресурсов','Осколки, книги и самоцветы позволяют новому владельцу развивать аккаунт.']].map(item=><article key={item[0]}><b>{item[0]}</b><h3>{item[1]}</h3><p>{item[2]}</p></article>)}</div></section></main>}
...
function Calculator({form,setForm}:{form:FormState;setForm:React.Dispatch<React.SetStateAction<FormState>>}){
  const[ready,setReady]=useState(false);const[generationArmed,setGenerationArmed]=useState(false);const result=useMemo(()=>estimate(form),[form]);
  useEffect(()=>{document.querySelector('.notice')?.setAttribute('id','data-check-notice')},[]);
  const update=<K extends keyof FormState>(key:K,value:FormState[K])=>setForm(current=>({...current,[key]:value}));
  const generateOffer=()=>{if(!generationArmed){setGenerationArmed(true);document.querySelector('#data-check-notice')?.scrollIntoView({behavior:'smooth',block:'center'});return}const selectedNames=form.champions.map(id=>champions.find(champion=>champion.id===id)?.name).filter(Boolean);const comboNames=result.foundCombos.map(combo=>combo.name);const clanBossLabels={normal:'обычный/сложный',brutal:'жестокий',nightmare:'кошмарный',ultra:'ультра-кошмарный'};const hydraLabels={none:'не указана',normal:'обычная',hard:'сложная',brutal:'жестокая',nightmare:'кошмарная'};const title=[comboNames[0]||selectedNames.slice(0,2).join(' + ')||'Аккаунт RAID',`${form.mythicals} миф.`,`${form.legendaries} лег.`,`ур. ${form.level}`,`${form.power}М`].join(' · ');const description=`Ключевые герои: ${selectedNames.join(', ')||'не указаны'}. Связки: ${comboNames.join(', ')||'не отмечены'}. Клановый босс: ${clanBossLabels[form.clanBoss]}. Гидра: ${hydraLabels[form.hydra]}. Коллекция: ${form.mythicals} мифических и ${form.legendaries} легендарных героев, ${form.sixStar} героев 6★. Уровень ${form.level}, сила ${form.power} млн. Энергия: ${fmt(form.energy)}, серебро: ${fmt(form.silver)}, рубины: ${fmt(form.gems)}. Войны фракций: ${form.factionWars} звёзд, Большой зал: ${form.greatHall}.`;window.dispatchEvent(new CustomEvent('rsl-generate-offer',{detail:{title,description}}));document.querySelector('#market')?.scrollIntoView({behavior:'smooth'})};
  ...
  return <section className="calculator" id="calculator"><div className="section-title"><div><span>02 / ПРОВЕРКА ДАННЫХ</span><h2>Проверьте автозаполнение</h2></div><p>Исправьте значения, которые система прочитала неточно.</p></div><div className="calc-grid"><form onSubmit={event=>{event.preventDefault();setReady(true)}}>
    ...
    <Fieldset n={2} title="Коллекция"><Field label="Легендарных героев" value={form.legendaries} onChange={value=>update('legendaries',value)}/><Field label="Мифических героев" value={form.mythicals} onChange={value=>update('mythicals',value)}/><Field optional label="Легендарных Войд" value={form.voidLegendaries} onChange={value=>update('voidLegendaries',value)}/><Field optional label="Героев 6★" value={form.sixStar} onChange={value=>update('sixStar',value)}/></Fieldset>
    <ChampionPicker selected={form.champions} onChange={value=>update('champions',value)}/>
    <Fieldset n={4} title="Прогресс"><Field optional label="Звёзды Войн фракций" value={form.factionWars} onChange={value=>update('factionWars',value)}/><Field optional label="Бонусы Большого зала" value={form.greatHall} onChange={value=>update('greatHall',value)}/><SelectField label="Клановый босс" value={form.clanBoss} onChange={value=>update('clanBoss',value as FormState['clanBoss'])} options={[{value:'normal',label:'Обычный / Сложный'},{value:'brutal',label:'Жестокий'},{value:'nightmare',label:'Кошмарный'},{value:'ultra',label:'Ультра-кошмарный'}]}/><SelectField label="Гидра" value={form.hydra} onChange={value=>update('hydra',value as FormState['hydra'])} options={[{value:'none',label:'Не указана'},{value:'normal',label:'Обычная'},{value:'hard',label:'Сложная'},{value:'brutal',label:'Жестокая'},{value:'nightmare',label:'Кошмарная'}]}/><SelectField label="Классическая арена" value={form.arena} onChange={value=>update('arena',value as FormState['arena'])} options={[{value:'bronze',label:'Бронза'},{value:'silver',label:'Серебро'},{value:'gold',label:'Золото'},{value:'platinum',label:'Платина'}]}/></Fieldset>
    <Fieldset n={5} title="Ресурсы"><Field label="Самоцветы / рубины" value={form.gems} onChange={value=>update('gems',value)}/><Field label="Энергия" value={form.energy} onChange={value=>update('energy',value)}/><Field label="Серебро" value={form.silver} onChange={value=>update('silver',value)}/><Field label="Сакральные осколки" value={form.sacred} onChange={value=>update('sacred',value)}/><Field optional label="Войд-осколки" value={form.voidShards} onChange={value=>update('voidShards',value)}/><Field optional label="Легендарные книги" value={form.legendaryBooks} onChange={value=>update('legendaryBooks',value)}/></Fieldset>
    ...
    <button className={`generate-offer ${generationArmed?'armed':''}`} type="button" onClick={generateOffer}>{generationArmed?'Данные проверены — сгенерировать':'Сгенерировать название и описание'} <span>→</span></button>
  </div></section>;
}
